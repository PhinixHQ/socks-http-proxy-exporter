...
// Gauges
let probeSuccessGuage = new client.Gauge({ name: 'proxy_probe_kia', help: 'status of proxy server'});
let probeDurationGuage = new client.Gauge({ name: 'proxy_probe_ducration_seconds', help: 'response time of proxy server'});
...
app.get('/probe', validator.query(querySchema), async (req, res) => {
    ...
    if(q.protocol.split(':')[0] == 'http'){
        const res = await scrapeWithHttpProxy(queryTarget,queryProxyHost,queryProxyPort,queryProxyAuth);
        ...
        const res = await scrapeWithSocksProxy(queryTarget,queryProxyHost,queryProxyPort);
        probeSuccessGuage.set(res.proxy_probe_success);
        probeDurationGuage.set(res.proxy_probe_duration_seconds);
        console.log(res);
        await timeout(10000);
